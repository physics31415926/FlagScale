...
from flagscale.agent.react.chip.base import (
    ChipCapability,
    OperatorSupport,
    PrecisionSupport,
...
from flagscale.agent.react.chip.registry import CHIP_REGISTRY, get_chip
from flagscale.agent.react.chip.detect import detect_chip
from flagscale.agent.react.chip.nvidia import NVIDIA_A100, NVIDIA_H100
from flagscale.agent.react.chip.migration import (
    MigrationDiff,
    MigrationItem,
...
class TestDetection:
    ...
    def test_env_override_unknown_vendor(self):
        with patch.dict(os.environ, {
            ...
            with patch(
                "flagscale.agent.react.chip.detect._detect_nvidia", return_value=None
            ):
                chip = detect_chip()
                assert chip is None
