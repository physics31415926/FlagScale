...
requires: [env-setup, train-config, train-data-prep, train-run]
suggests: [topo-detect, train-parallel-strategy, train-precision-alignment]
activate_on: [is_migration]

effects:
  mode: porting
  initial_phase: analysis
  companion_skills: [train-parallel-strategy, train-precision-alignment, train-config, train-run, env-setup, workspace-layout, train-data-prep]

workflow:
  trigger:
    keywords: [migrate, port, 迁移, porting, convert]
...
constraints:
  - id: no_dummy_data
    ...
    trigger:
      ...
      keywords: [torch.rand, torch.zeros, torch.ones, dummy_input, fake_data]
  ...
  - id: read_before_write
    ...
    trigger:
      tools: [write_file]
      keywords: [class, def, Module, forward]
...
warnings:
  - id: frozen_native_impl
    ...
    severity: warning
    trigger:
      keywords: [frozen, freeze, requires_grad=False, no_grad, feature_extractor]
    prompt: "Check if agent is skipping native implementation for a frozen component"
    reminder: "Frozen ≠ skip native. ALL components must use Megatron primitives regardless of training status."
    max_reminders: 2
  - id: parallelism_assessment
    ...
    trigger:
      tools: [write_file, edit_file]
      keywords: [ColumnParallelLinear, RowParallelLinear, tensor_model_parallel]
    prompt: "Check if parallelism feasibility was assessed for this component"
    reminder: "Assess each component independently for TP benefit. Small MLPs may not need TP."
    max_reminders: 1

context_injection:
  always: ["Important Notes"]
  by_stage:
    analyze: ["Porting Discipline", "Pre-coding analysis", "Porting Modes"]
    implement: ["Implementation Flow", "Failure Pivot Discipline", "Multimodal Module Nesting", "get_batch Under Parallelism"]
    verify: ["Verification Standard"]
  by_tool:
    write_file: ["Implementation Flow", "Failure Pivot Discipline"]
    shell: ["Verification Standard"]
---
