---
name: topo-detect
description: Detect hardware topology — NVLink, NUMA, RDMA, disks on single node. For multi-node, also run NCCL tests to verify cross-node communication bandwidth. Generates a YAML report with parallelism recommendations.
keywords:
  - topology
  - topo
  - hardware
...
- `env-setup` — install FlagScale environment on detected hardware
